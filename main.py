...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        mydb = mysql.connector.connect(host="localhost", user="root", password="nziza", database="loginapp")
...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        print(username)
        mycursor.execute(
            'SELECT * FROM LoginDetails WHERE username = %s AND email_id = %s', (username, email)
        )
        account = mycursor.fetchone()
        print(account)
        if account:
            ...
            mycursor.execute('INSERT INTO LoginDetails (username, email_id, password) VALUES (%s, %s, %s)', (username, email, password))
            mydb.commit()
